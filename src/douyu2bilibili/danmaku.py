...
logger = logging.getLogger("pipeline.danmaku")
    ...
    for flv_file in flv_files:
        try:
            ...
            if file_size < min_size_bytes:
                ...
                logger.info(f"找到小于 {config.MIN_FILE_SIZE_MB}MB 的文件: {os.path.basename(flv_file)} ({file_size / (1024*1024):.2f}MB)")

                # 1. 删除 flv 文件
                try:
                    os.remove(flv_file)
                    logger.info(f"已删除: {os.path.basename(flv_file)}")
                    files_deleted += 1
                    ...
                    if os.path.exists(xml_file):
                        try:
                            os.remove(xml_file)
                            logger.info(f"已删除对应的 XML: {os.path.basename(xml_file)}")
                        except OSError as e:
                            logger.error(f"删除 XML 文件 {os.path.basename(xml_file)} 失败: {e}")
                    else:
                        logger.warning(f"未找到 {os.path.basename(flv_file)} 对应的 XML 文件: {os.path.basename(xml_file)}")

                except OSError as e:
                    logger.error(f"删除 FLV 文件 {os.path.basename(flv_file)} 失败: {e}")

        except FileNotFoundError:
            logger.warning(f"检查文件大小时未找到文件: {flv_file} (可能已被其他进程处理)")
        except Exception as e:
            logger.error(f"处理文件 {os.path.basename(flv_file)} 时出错: {e}")

    logger.info(f"小文件清理完成，共删除 {files_deleted} 个 FLV 文件及其对应 XML。")
...
def get_video_resolution(video_file):
    ...
    try:
        ...
        if data and 'streams' in data and data['streams']:
            ...
            if width and height:
                logger.info(f"获取到视频 {os.path.basename(video_file)} 分辨率: {width}x{height}")
                return int(width), int(height)
        logger.error(f"无法从 ffprobe 输出中解析分辨率: {video_file}")
        return None, None
    except FileNotFoundError:
        logger.error(f"找不到 ffprobe 命令，请检查 config.py 中的 FFPROBE_PATH 设置或确保 ffprobe 在系统 PATH 中。")
        return None, None
    except subprocess.CalledProcessError as e:
        logger.error(f"运行 ffprobe 时出错 (文件: {os.path.basename(video_file)}): {e}")
        logger.error(f"ffprobe stderr: {e.stderr}")
        return None, None
    except json.JSONDecodeError as e:
        logger.error(f"解析 ffprobe JSON 输出时出错 (文件: {os.path.basename(video_file)}): {e}")
        logger.error(f"ffprobe stdout: {result.stdout}")
        return None, None
    except Exception as e:
         logger.error(f"获取视频分辨率时发生未知错误 (文件: {os.path.basename(video_file)}): {e}")
         return None, None


def convert_danmaku():
    """转换 PROCESSING_FOLDER 中的 XML 文件为 ASS 文件，跳过正在录制的视频"""
    logger.info("开始转换 XML 弹幕文件为 ASS...")
    converted_count = 0
    skipped_count = 0
    error_count = 0
    ...
    for xml_file in xml_files:
        ...
        if _failure_counts.get(xml_file, 0) >= config.MAX_RETRY_COUNT:
            logger.info(f"文件 {os.path.basename(xml_file)} 已达失败阈值，跳过")
            skipped_count += 1
            continue

        # 检查是否存在对应的 .flv.part 文件，如果存在则跳过
        if os.path.exists(flv_part_file):
            logger.info(f"跳过转换，因为找到正在录制的文件: {os.path.basename(flv_part_file)}")
            skipped_count += 1
            continue

        # 检查是否存在对应的 .flv 文件
        if not os.path.exists(flv_file):
            logger.warning(f"跳过转换，因为找不到对应的 FLV 文件: {os.path.basename(flv_file)} (XML: {os.path.basename(xml_file)})")
            skipped_count += 1
            continue

        # 检查是否已存在 ass 文件
        if os.path.exists(ass_file):
             logger.info(f"ASS 文件已存在，跳过转换: {os.path.basename(ass_file)}")
             skipped_count +=1
             continue
        ...
        if resolution_x is None or resolution_y is None:
            logger.error(f"无法获取视频分辨率，跳过转换: {os.path.basename(flv_file)}")
        ...
        # 执行转换
        try:
            logger.info(f"正在转换: {os.path.basename(xml_file)} -> {os.path.basename(ass_file)}")
            convert_xml_to_ass(
            ...
            if os.path.exists(ass_file):
                 ...
                 logger.info(f"成功转换: {os.path.basename(ass_file)}")
                 
                 # 删除XML文件 (根据配置决定是否删除)
                 if config.DELETE_UPLOADED_FILES:
                     try:
                         os.remove(xml_file)
                         logger.info(f"已删除原始 XML 文件: {os.path.basename(xml_file)} (根据配置)")
                     except OSError as e:
                         logger.warning(f"删除 XML 文件 {os.path.basename(xml_file)} 失败: {e}")
                 else:
                     logger.info(f"保留原始 XML 文件: {os.path.basename(xml_file)} (根据配置)")
                 
                 _clear_failure(xml_file)
                 converted_count += 1
            else:
                 logger.error(f"转换函数执行完毕但未找到输出文件: {os.path.basename(ass_file)}")
                 _record_failure(xml_file, flv_file)
                 error_count += 1

        except Exception as e:
            logger.error(f"转换 XML 文件 {os.path.basename(xml_file)} 时出错: {e}")
            _record_failure(xml_file, flv_file)
            error_count += 1

    logger.info(f"弹幕转换完成。成功: {converted_count}, 跳过: {skipped_count}, 失败: {error_count}")
